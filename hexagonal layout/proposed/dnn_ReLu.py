...
class Dnn:
    def __init__(self, batch_size=1, n_epoch=500, layer_dim_list=[16, 16, 3], num_cell=19, num_user_per_cell=10,
                 N0=1e-9, W=1e6, beta_min=1, beta_max=16, Cthr=1e6, alpha=1):
    ...
    def get_itfc_indicator_hex(self, is_center):

        BP_green = np.array([2, 4, 6, 10, 14, 18, 21, 25, 27, 31, 33, 37])
        ...
        BP_blue = np.array([3, 5, 7, 8, 12, 16, 22, 24, 28, 30, 34, 36])
        ...
        BP_red = np.array([1, 9, 11, 13, 15, 17, 19, 20, 23, 26, 29, 32, 35])
    ...
    def Calc_capa(self, input_h, Ptotal, beta):

        input_h_not_scaled = np.reshape(input_h, [self.num_cell, self.num_user_per_cell, self.num_cell])

        [all_c_indicator_itfc_bp, all_c_indicator_itfc_p] = self.get_itfc_indicator_hex('center')
        [all_e_indicator_itfc_bp, all_e_indicator_itfc_p] = self.get_itfc_indicator_hex('edge')
        ...
        for c_idx in range(self.num_cell):
            ...
            for u_idx in range(self.num_user_per_cell):
                ...
                for i in range(self.num_cell):
                    if i != c_idx:
                        itfc_C = Ptotal[i] / (self.num_center * (2 + beta[i]))
                        c_tmp1 = h[i] * itfc_C * c_indicator_itfc_p[:, i]
                        c_tmp2 = h[i] * beta[i] * itfc_C * c_indicator_itfc_bp[:, i]
                        Ic = Ic + (c_tmp1 + c_tmp2)

                        e_tmp1 = h[i] * itfc_C * e_indicator_itfc_p[i]
                        e_tmp2 = h[i] * beta[i] * itfc_C * e_indicator_itfc_bp[i]
                        Ie = Ie + (e_tmp1 + e_tmp2)
                ...
                sv_C = Ptotal[c_idx] / (self.num_center * (2 + beta[c_idx]))

                SINR_c = k0 * sv_C * h[c_idx] / (noise_power_c + itfc_power_c)
                SINR_e = k0 * beta[c_idx] * sv_C * h[c_idx] / (noise_power_e + itfc_power_e)
    ...
    def train_dnn(self, input_h, lr, file_path):
        ...
        with tf.device('/CPU:0'):
            ...
            for i in range(len(self.layer_dim_list)):
                ...
                if i < len(self.layer_dim_list) - 1:
                    ...
                    mult_beta = mult[:, :self.num_cell]
                    mult_Ptotal = mult[:, -self.num_cell:]

                    output_beta = (self.beta_max - self.beta_min) * tf.nn.sigmoid(mult_beta) + self.beta_min

                    output_Ptotal = tf.nn.sigmoid(mult_Ptotal) + 1e-100
            ...
            for c_idx in range(self.num_cell):
                ...
                for u_idx in range(self.num_user_per_cell):
                    ...
                    for i in range(self.num_cell):
                        if i != c_idx:
                            itfc_C = output_Ptotal[:, i] / (self.num_center * (2 + output_beta[:, i]))
                            c_tmp1 = tf.expand_dims(h[:, i], axis=1) * tf.expand_dims(itfc_C, axis=1) * tf.reshape(c_indicator_itfc_p[:, i], [1, -1])
                            c_tmp2 = tf.expand_dims(output_beta[:, i], axis=1) * tf.expand_dims(itfc_C,axis=1) * tf.expand_dims(h[:, i], axis=1) * tf.reshape(c_indicator_itfc_bp[:, i], [1,-1])
                            Ic = Ic + (c_tmp1 + c_tmp2)

                            e_tmp1 = h[:, i] * itfc_C * e_indicator_itfc_p[i]
                            e_tmp2 = output_beta[:, i] * itfc_C * h[:, i] * e_indicator_itfc_bp[i]
                            Ie = Ie + (e_tmp1 + e_tmp2)
                    ...
                    sv_C = output_Ptotal[:, c_idx] / (self.num_center * (2 + output_beta[:, c_idx]))

                    SINR_c = tf.expand_dims(k0 * sv_C * h[:, c_idx], axis=1) / (noise_power_c + itfc_power_c)
                    SINR_e = (k0 * output_beta[:, c_idx] * sv_C * h[:, c_idx]) / (noise_power_e + itfc_power_e)
            ...
            for c_idx in range(self.num_cell):
                ...
                tmp_c_capa = tf.reshape(tf.gather_nd(c_capa_per_cell[c_idx, :, :], tst3), [-1, self.num_center])  # (batch_size, num_center)
                ...
                tmp_sum_capa = tf.reduce_sum(tmp_c_capa, axis=1, keepdims=True) + tf.reduce_sum(tmp_e_capa, axis=1, keepdims=True)  # axis=1 행 단위합
            ...
            dnn_beta_per_sample = sess.run(output_beta,feed_dict={x_ph: input_h})
            ...
            for m in range(input_h.shape[0]):
                dnn_sum_capa_per_sample[m, :, :], dnn_is_center_per_sample[m, :, :] = self.Calc_capa(input_h_not_scaled[m, :], dnn_Ptotal_per_sample[m, :], dnn_beta_per_sample[m, :])
        ...
        for m in range(input_h.shape[0]):
            ...
            with open(lr_path + '\dnn_beta_train.dat', 'a') as f_b:
                f_b.write('%g\t' % (lr))
                for c in range(self.num_cell):
                    f_b.write('%10.10g  ' % dnn_beta_per_sample[m, c])
                f_b.write('\n')
    ...
    def test_dnn(self, input_h, lr, file_path):
        ...
        for m in range(input_h.shape[0]):
            ...
            with open(lr_path + '\dnn_beta_test.dat', 'a') as f_b:
                f_b.write('%g\t' % (lr))
                for c in range(self.num_cell):
                    f_b.write('%10.10g  ' % dnn_beta_per_sample[m, c])
                f_b.write('\n')
