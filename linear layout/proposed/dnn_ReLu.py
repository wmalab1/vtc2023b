...
class Dnn:
    def __init__(self, batch_size=1, n_epoch=500, layer_dim_list=[16, 16, 3], num_cell=7, num_user_per_cell=16,
                 N0=1e-9, W=1e7, beta_min=1, beta_max=16, Cthr=1e6, alpha=1):
    ...
    def train_dnn(self, input_h, lr, file_path):
        ...
        with tf.device('/CPU:0'):
            ...
            capacity_prime = sum_capa - self.alpha * (tf.nn.relu(self.Cthr-center_capa_per_batch) + tf.nn.relu(self.Cthr-edge_capa_per_batch))
        ...
        with tf.device('/CPU:0'):
            ...
            for i in range(len(self.layer_dim_list)):
                ...
                if i < len(self.layer_dim_list) - 1:
                    ...
                    output_beta = (self.beta_max - self.beta_min) * tf.nn.sigmoid(mult_beta) + self.beta_min
                    print('output_beta', output_beta.shape)

                    output_Ptotal = tf.nn.sigmoid(mult_Ptotal) + 1e-100
                    print('output_Ptotal', output_Ptotal.shape)
